...
int crs_read(MPI_Comm comm,
    ...
    CATCH_MPI_ERROR(
        MPI_File_read_at_all(file, offset * rowptr_type_size, crs->rowptr, nlocal + 1, rowptr_type, &status));
    ...
    ptrdiff_t end = to_ptrdiff_t(rowptr_type, &(rowptr_bytes[nlocal * rowptr_type_size]));
    ...
    CATCH_MPI_ERROR(MPI_File_read_at_all(file, start * colidx_type_size, crs->colidx, nnz, colidx_type, &status));
    ...
    CATCH_MPI_ERROR(MPI_File_read_at_all(file, start * values_type_size, crs->values, nnz, values_type, &status));
